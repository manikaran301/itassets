...
async function main() {
  console.log('--- Database Admin Account Creation ---');
  const passwordHash = await bcrypt.hash('Admin@1221', 10);

  for (const user of users) {
    try {
      const result = await prisma.systemUser.upsert({
      ...
      console.log(`✅ Success: ${result.fullName} (User: ${result.username})`);
    } catch (error) {
      console.error(`❌ Error for ${user.name}:`, error.message);
    }
  }
  console.log('--- All accounts successfully processed ---');
}
