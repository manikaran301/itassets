...
export default function AssetsPage() {
  ...
  const stats = [
    ...
    {
      label: "In Repair",
      value: assets.filter((a) => a.status === "in_repair").length,
      icon: Wrench,
    ...
    <div className="space-y-4 animate-fade-in max-w-full pb-20 text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {stats.map((stat, i) => (
          <div
            key={i}
            className="p-3 rounded-xl bg-card border border-border premium-card flex items-center justify-between group"
          >
            <div>
            ...
            <div
              className={cn(
                "w-10 h-10 rounded-lg flex items-center justify-center border transition-transform group-hover:scale-110 duration-500",
                stat.color,
              )}
            >
          ...
          <table className="w-full text-left border-collapse min-w-[1200px]">
            <thead>
              <tr className="bg-white/[0.02] border-b border-white/5">
                <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                  Asset Tag & Type
                </th>
                <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                  Make & Model
                ...
                <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-muted-foreground text-center">
                  Status
                </th>
                <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-muted-foreground text-right pr-6">
                  Actions
                </th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-0.5 min-w-0">
                          <p className="text-sm font-black group-hover:text-primary transition-colors truncate uppercase tracking-tight">
                            {asset.assetTag}
                          </p>
                          <p className="text-[9px] font-black tracking-widest text-muted-foreground/80 uppercase">
                            {asset.type}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3">
                      <div className="space-y-0.5">
                        <p className="text-[11px] font-black text-foreground/80 tracking-tight">
                          {asset.make || "—"}
                        </p>
                        <p className="text-[9px] text-muted-foreground font-black italic truncate max-w-[180px]">
                          {asset.model || "—"}
                        </p>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1.5">
                        {asset.cpu && (
                          <span className="text-[8px] bg-muted px-2 py-0.5 rounded border border-border/50 text-muted-foreground uppercase tracking-widest font-black truncate max-w-[120px]">
                            {asset.cpu}
                          </span>
                        )}
                        {asset.ramGb && (
                          <span className="text-[8px] bg-primary/5 px-2 py-0.5 rounded border border-primary/10 text-primary uppercase tracking-widest font-black">
                            {asset.ramGb}GB
                          </span>
                        )}
                        {asset.ssdGb && (
                          <span className="text-[8px] bg-secondary/5 px-2 py-0.5 rounded border border-secondary/10 text-secondary uppercase tracking-widest font-black">
                            {asset.ssdGb}GB SSD
                          </span>
                        )}
                        {asset.hddGb && (
                          <span className="text-[8px] bg-accent/5 px-2 py-0.5 rounded border border-accent/10 text-accent uppercase tracking-widest font-black">
                            {asset.hddGb}GB HDD
                          </span>
                        )}
                        {!asset.cpu &&
                          ...
                          !asset.ssdGb &&
                          !asset.hddGb && (
                            <span className="text-[9px] text-muted-foreground/20 italic">
                              No specs
                            </span>
                          )}
                      </div>
                      ...
                      <div className="space-y-1">
                        {asset.ipAddress ? (
                          <div className="flex items-center gap-1.5 group/ip">
                            <p
                              className="text-[10px] font-mono text-foreground/90 font-bold truncate max-w-[120px]"
                    ...
                    <td className="px-4 py-3">
                      {asset.currentEmployee ? (
                        <div className="flex items-center gap-3">
                          ...
                          <div className="space-y-0.5 min-w-0">
                            <p className="text-[11px] font-black text-foreground/90 truncate max-w-[120px]">
                              {asset.currentEmployee.fullName}
                            </p>
                        ...
                        <span className="text-[10px] text-muted-foreground/60 italic font-black">
                          Unassigned
                        </span>
                      )}
                    </td>
