...
export default function EmailAccountsPage() {
  ...
  const getPlatformIcon = (platform: string) => {
    switch (platform) {
      case 'google_workspace': return <Globe className="w-3.5 h-3.5 text-blue-500" />;
      case 'microsoft_365': return <Server className="w-3.5 h-3.5 text-blue-600" />;
      case 'zoho': return <Mail className="w-3.5 h-3.5 text-red-500" />;
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/[0.03] bg-muted/20">
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Identity & Address</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Status</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Platform</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Linked Associate</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-right">Actions</th>
              </tr>
            </thead>
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="py-20 text-center">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground animate-pulse">Synchronizing Identities...</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={5} className="py-20 text-center text-muted-foreground">
                    ...
                    <p className="text-sm font-bold opacity-30">No matching email accounts found.</p>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={email.id} className="group hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-0.5">
                          <p className="text-xs font-black tracking-tight group-hover:text-primary transition-colors">{email.emailAddress}</p>
                          <div className="flex items-center gap-2">
                             <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest opacity-60">{email.displayName}</p>
                             <span className="w-1 h-1 rounded-full bg-white/10" />
                             <p className="text-[10px] text-muted-foreground/40 font-bold uppercase tracking-widest">{email.accountType}</p>
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <span className={cn(
                        ...
                        <div className={cn("w-1 h-1 rounded-full", email.status === 'active' ? "bg-green-500 animate-pulse" : "bg-current")} />
                        {email.status}
                      </span>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-0.5">
                          <p className="text-[10px] font-black uppercase tracking-tight">{email.employee?.fullName}</p>
                          <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-widest opacity-40">{email.employee?.employeeCode}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="p-2 hover:bg-primary/10 text-muted-foreground hover:text-primary rounded-xl transition-all" title="Manage Account">
                          <Edit2 className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-2 hover:bg-primary/10 text-muted-foreground hover:text-primary rounded-xl transition-all" title="Forwarding Rules">
                          <RefreshCw className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-2 hover:bg-red-500/10 text-muted-foreground hover:text-red-500 rounded-xl transition-all" title="Suspend Access">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-primary/5 rounded-[24px] border border-primary/10">
        <div className="flex items-center gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
            Registry synchronized with <span className="text-primary">Corporate Directory</span>
          </p>
        </div>
