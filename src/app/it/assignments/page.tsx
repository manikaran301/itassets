...
export default async function AssignmentsPage() {
  ...
  const getTypeStyle = (type: string) => {
    switch (type) {
      ...
      case 'repair_return': return 'bg-secondary/10 text-secondary border-secondary/20';
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-card p-4 rounded-2xl premium-card border-border/50">
        <p className="text-xs text-muted-foreground font-semibold">
          Showing {assignments.length} assignment logs from live database records.
        </p>
      </div>
            ...
            {assignments.map((log) => {
              ...
              <tr key={log.id} className="hover:bg-muted/20 transition-all group">
                <td className="px-6 py-5">
                  <span className="text-xs font-mono font-bold bg-muted px-2 py-1 rounded border border-border group-hover:bg-primary/10 transition-colors uppercase">{log.logCode}</span>
                </td>
                <td className="px-6 py-5">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-card border border-border flex items-center justify-center text-primary">
                      {log.assetCategory === 'asset' ? <Monitor className="w-4 h-4" /> : <HardDrive className="w-4 h-4" />}
                    </div>
                    <div className="space-y-0.5">
                      <p className="text-sm font-bold tracking-tight">{log.asset?.assetTag ?? log.accessory?.assetTag ?? "-"}</p>
                      <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">{log.assetCategory}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-5">
                ...
                <td className="px-6 py-5 border-l border-border/10 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <button className="p-2 hover:bg-primary/10 hover:text-primary rounded-lg transition-all" title={status}>
                      <ExternalLink className="w-4 h-4" />
                    </button>
                    {status === "Returned" ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : status === "In Repair" ? <Wrench className="w-4 h-4 text-accent" /> : <MoreVertical className="w-4 h-4" />}
                  </div>
                </td>
              </tr>
            )})}
