    ...
    <div className="space-y-4 animate-fade-in pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {metricCards.map((card) => (
          <div
            key={card.label}
            className="premium-card rounded-xl border border-border bg-card p-4 relative overflow-hidden group"
          >
            <div
              className={cn(
                "absolute -right-4 -top-4 w-16 h-16 rounded-full opacity-10 blur-xl group-hover:scale-150 transition-transform duration-500",
                card.color,
              )}
            />
            ...
            <p className="text-[9px] font-semibold text-muted-foreground/60 mt-3 border-t border-border/50 pt-2">
              {card.detail}
            </p>
          </div>
        ))}
        ...
        <section className="xl:col-span-2 space-y-4">
          ...
          <div className="premium-card rounded-xl border border-border bg-card p-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                {
                  label: "Available",
                  value: assetStatusCounts.available,
                  tone: "border-green-500/20 bg-green-500/5 text-green-600",
                },
                {
                  label: "Assigned",
                  value: assetStatusCounts.assigned,
                  tone: "border-primary/20 bg-primary/5 text-primary",
                },
                {
                  label: "In Repair",
                  value: assetStatusCounts.inRepair,
                  tone: "border-accent/20 bg-accent/5 text-accent",
                },
                {
                  label: "Retired",
                  value: assetStatusCounts.retired,
                ...
                >
                  ...
                  <p className="text-[8px] font-black uppercase tracking-widest mt-1 opacity-80">
                    {item.label}
                  </p>
                </div>
              ))}
          ...
          <div className="premium-card rounded-xl border border-border bg-card p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {deviceMix.length === 0 ? (
                <p className="text-[10px] text-muted-foreground italic col-span-3">
                  No assets available yet.
                </p>
              ) : (
                deviceMix.map(([type, count]) => {
                  const Icon =
                    deviceIcons[type as keyof typeof deviceIcons] ?? Briefcase;
                  const percentage =
                    Math.round((count / assets.length) * 100) || 0;
                  return (
                    <div
                      key={type}
                      className="rounded-lg border border-border/50 bg-muted/10 p-3 flex items-center justify-between"
                    >
        ...
        <aside className="space-y-4">
          <div className="premium-card rounded-xl border border-border bg-card p-4">
            <h3 className="text-xs font-black uppercase tracking-widest mb-3 flex items-center gap-2">
              <Briefcase className="w-3.5 h-3.5 text-accent" /> Department
              Spread
            </h3>
            <div className="space-y-1.5">
              {departmentMix.map(([department, count]) => (
                <div
                  key={department}
                  className="flex items-center justify-between rounded p-2 border border-transparent hover:border-border/50 hover:bg-muted/30 transition-colors"
                >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="premium-card rounded-xl border border-border bg-card p-4">
              <h3 className="text-[9px] font-black uppercase tracking-widest mb-3 text-muted-foreground">
                Platforms
              </h3>
              <div className="space-y-1">
                {emailPlatforms.map(([platform, count]) => (
                  ...
                  >
                    <p
                      className="text-[9px] font-bold uppercase truncate pr-2"
                      title={platform}
                    >
                      {platform.replace("_", " ")}
                    </p>
              ...
              <div className="space-y-1">
                {roleMix.map(([role, count]) => (
                  <div
                    ...
                    className="flex justify-between items-center py-1 border-b border-border/50 last:border-0"
                  >
                    <p className="text-[9px] font-bold uppercase truncate pr-2">
                      {role}
                    </p>
                    <p className="text-[10px] font-black">{count}</p>
                  </div>
          ...
          <div className="premium-card rounded-xl border border-border bg-card p-4">
            <h3 className="text-[9px] font-black uppercase tracking-widest mb-3 text-muted-foreground border-b border-border/50 pb-2">
              ...
              {auditLogs.map((log) => (
                ...
                >
                  ...
                  <p className="text-[9px] font-bold text-muted-foreground truncate flex-1">
                    ...
                    on {log.entityType.replaceAll("_", " ")}
                  </p>
                </div>
              ))}
            </div>
