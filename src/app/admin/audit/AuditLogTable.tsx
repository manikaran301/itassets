        ...
        <table className="w-full border-collapse text-left text-xs">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="px-4 py-3 font-bold uppercase tracking-widest text-muted-foreground/60">Event</th>
              <th className="px-4 py-3 font-bold uppercase tracking-widest text-muted-foreground/60">Action</th>
              <th className="px-4 py-3 font-bold uppercase tracking-widest text-muted-foreground/60">User</th>
              <th className="px-4 py-3 font-bold uppercase tracking-widest text-muted-foreground/60">Summary</th>
              <th className="px-4 py-3 font-bold uppercase tracking-widest text-muted-foreground/60 text-right">Time</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border/50">
            {logs.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-24 text-center text-muted-foreground font-medium italic">
          ...
          <div className="relative w-full max-w-2xl bg-card border border-border rounded-[32px] shadow-[0_32px_128px_-16px_rgba(0,0,0,0.5)] overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-500">
            ...
            <div className="flex items-center justify-between p-6 border-b border-border bg-muted/30">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary border border-primary/20 shadow-inner">
                  {(() => {
                    const Icon = entityIcons[selectedLog.entityType] ?? FileText;
            ...
            <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl border border-border bg-muted/10">
                  <div className="flex items-center gap-2 mb-2 text-muted-foreground">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Operation Type</span>
                  </div>
                  <span className={cn(
                ...
                <div className="p-4 rounded-2xl border border-border bg-muted/10">
                  <div className="flex items-center gap-2 mb-2 text-muted-foreground">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Authenticated User</span>
                  </div>
                  <p className="text-sm font-black">{selectedLog.user?.fullName || "System"}</p>
                  <p className="text-[10px] text-muted-foreground uppercase opacity-60">@{selectedLog.user?.username || "automation"}</p>
                </div>
                <div className="p-4 rounded-2xl border border-border bg-muted/10">
                  <div className="flex items-center gap-2 mb-2 text-muted-foreground">
                    <Clock className="w-3.5 h-3.5" />
                    <span className="text-[10px] font-black uppercase tracking-widest">Occurred At</span>
                  </div>
                  <p className="text-sm font-black">{new Date(selectedLog.changedAt).toLocaleString()}</p>
                  <p className="text-[10px] text-muted-foreground italic font-medium">{formatDistanceToNow(new Date(selectedLog.changedAt), { addSuffix: true })}</p>
                </div>
                <div className="p-4 rounded-2xl border border-border bg-muted/10">
                  <div className="flex items-center gap-2 mb-2 text-muted-foreground">
                  ...
                  <p className="text-[10px] font-black font-mono break-all">{selectedLog.entityId}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-3 px-1">Detailed Change Data</h3>
                  <div className="rounded-[24px] border border-white/5 bg-zinc-950 p-6 overflow-x-auto shadow-2xl relative group/code">
                    <div className="absolute top-4 right-4 text-[9px] font-black text-white/20 uppercase tracking-widest group-hover/code:text-primary transition-colors">JSON SNAPSHOT</div>
