    ...
    <div className="space-y-6 animate-fade-in text-sm relative pb-20">
      ...
      <div className="bg-primary/5 border border-primary/10 rounded-[28px] p-6 mb-8 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden group">
         ...
         <div className="space-y-1 text-center md:text-left">
            <h2 className="text-xl font-bold tracking-tight">Onboarding Tracker: How it Works</h2>
            <p className="text-xs text-muted-foreground/80 font-medium leading-relaxed max-w-3xl">
               This page tracks the **Onboarding Progress** for every employee you just added. It shows you if their **Hardware** is ready, if their **Seat** is assigned, and if their **Email Account** is created. Until all 4 bars turn green, they stay in this "New Joiner" pipeline.
            </p>
         </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
         {[
           { icon: ShieldCheck, label: 'Identity', desc: 'HR Verification', color: 'primary' },
           { icon: Laptop, label: 'Hardware', desc: 'IT Asset Dispatch', color: 'secondary' },
           { icon: MapPin, label: 'Seating', desc: 'Desk Allocation', color: 'accent' },
           { icon: Mail, label: 'Access', desc: 'Email Generation', color: 'green-500' },
         ].map((step, i) => (
           <div key={i} className="p-4 rounded-[20px] bg-card border border-white/5 flex items-center gap-4 hover:border-white/10 transition-colors">
              <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center", `bg-${step.color}/10 text-${step.color}`)}>
                 <step.icon className="w-5 h-5" />
              </div>
              <div className="space-y-0.5">
                 <p className="text-[10px] font-black uppercase tracking-widest opacity-40 leading-none">{step.label}</p>
          ...
          <table className="w-full text-left border-collapse min-w-[1100px]">
             <thead>
                <tr className="bg-white/[0.02] border-b border-white/5">
                   <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Employee Identity</th>
                   <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Joining Location</th>
                   <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-center">Current Fulfillment Pipeline (Status)</th>
                   <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-right font-mono italic">Joining on</th>
                </tr>
             </thead>
                          ...
                          {Object.entries(joiner.steps).map(([key, label], idx) => (
                            <div key={key} className="flex flex-col items-center gap-2 flex-1">
                               ...
                               <div className={cn(
                                 ...
                                 label === 'Ready' || label === 'Allocated' ? "bg-green-500/10 border-green-500/20 text-green-500" :
                                 label === 'Procuring' || label === 'Pending' || label === 'Awaiting IT' ? "bg-primary/10 border-primary/20 text-primary animate-pulse" :
                                 label === 'Skip' ? "bg-muted/10 border-white/5 opacity-30 text-muted-foreground line-through" :
