  ...
  useEffect(() => {
    if (!isOpen) {
      if (selectedOption) {
        ...
        setSearch(value); // Keep the raw value visible if it's custom or options are still loading
      } else {
        setSearch('');
      }
    }
      ...
      <div ref={containerRef} className="relative">
        ...
        <div 
          className={cn(
            ...
            isOpen ? "border-primary/45 shadow-xl shadow-primary/10 ring-4 ring-primary/10" : "border-border/70 hover:border-border"
          )}
          onClick={() => {
            setIsOpen(true);
        ...
        >
          {icon && <span className={cn("shrink-0 transition-colors", isOpen ? "text-primary" : "text-muted-foreground/70")}>{icon}</span>}
          <input
            ref={inputRef}
            type="text"
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((opt) => (
                  ...
                  "w-full px-6 py-3 text-left text-[11px] font-bold flex items-center justify-between transition-all hover:bg-primary/5 group/opt mx-2 w-[calc(100%-16px)] rounded-2xl",
                      value === opt.value ? "bg-primary/10 text-primary" : "text-foreground/90 hover:text-foreground"
                    )}
                  >
